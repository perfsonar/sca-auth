...
router.post(
    ...
    function (req, res, next) {
        db.User.findOne({ where: { id: req.user.sub } }).then(function (user) {
            ...
            if (req.body.scopes)
                user.scopes = common.intersect_scopes(
                    user.scoppes,
                    req.body.scopes
                );
...
router.post("/send_email_confirmation", function (req, res, next) {
    db.User.findOne({ where: { id: req.body.sub } }).then(function (user) {
        ...
        common.send_email_confirmation(
            req.headers.referer,
            user,
            function (err) {
                if (err) return next(err);
                res.json({
                    message:
                        ...
                        config.local.email_confirmation.subject,
                });
            }
        );
    });
...
router.post("/confirm_email", function (req, res, next) {
    db.User.findOne({
        where: { email_confirmation_token: req.body.token },
    }).then(function (user) {
        if (!user)
...
router.put(
    ...
    function (req, res, next) {
        ...
        db.Group.findOne({ where: { id: req.params.id } }).then(function (
            group
        ) {
            ...
            group.getAdmins().then(function (admins) {
                ...
                if (
                    !~req.user.scopes.sca.indexOf("admin") &&
                    !~admin_ids.indexOf(req.user.sub)
                )
                    return res.send(401);
                //then update everything
                group
                    .update(req.body.group)
                    .then(function (err) {
                        group.setAdmins(req.body.admins).then(function () {
                            group
                                .setMembers(req.body.members)
                                .then(function () {
                                    res.json({
