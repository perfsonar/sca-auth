        ...
        passport.authenticate("google", function (err, user, info) {
            ...
            if (req.user) {
                ...
                if (user) {
                    ...
                    return res.redirect(
                        "/auth/#!/settings/account?msg=The github account is already associated to a SCA account"
                    );
                }
                ...
                if (!user) {
                    return res.redirect(
                        "/auth/#!/signin?msg=" +
                            "Your google account is not registered to SCA yet. Please login using your username/password first, then associate your google account inside account settings."
                    );
                }
                common.createClaim(user, function (err, claim) {
                    if (err) return next(err);
...
router.get(
    ...
    function (req, res, next) {
        ...
        passport.authenticate("google", {
            scope: ["profile"] /*callbackURL: callbackurl*/,
        })(req, res, next);
    }
);
