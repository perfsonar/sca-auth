    ...
    transporter.sendMail(
        {
            ...
            text:
                "Hello!\n\nIf you have requested to reset your password, please visit " +
                fullurl +
                " to reset your password (using the same browser you've used to send the request",
        },
        function (err, info) {
            if (err) return cb(err);
            if (info && info.response)
...
exports.setJwtCookies = function (claim, res) {
    ...
    res.cookie("XSRF-TOKEN", claim.xsrf, {
        domain: ".ppa.iu.edu",
        secure: true,
        path: "/",
    });
